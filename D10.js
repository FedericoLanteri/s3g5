...
function deleteOne(text,isFirst){
  if(isFirst){
    return text.slice(1)
  }
  else{
...
function isTodayMyBirthday(birthday) {
  ...
  return today.getMonth() + 1 === parseInt(birthMonth) && today.getDate() === parseInt(birthDay);
}

// Esempio di utilizzo:
const myBirthday = '10-25'; // Inserire il proprio compleanno nel formato MM-DD
console.log(isTodayMyBirthday(myBirthday)); // Questo stamperà true se oggi è il tuo compleanno, altrimenti false
...
function newestMovie(movies) {
  return movies.reduce((newest, movie) => {
    return (newest.Year > movie.Year) ? newest : movie;
  });
}
